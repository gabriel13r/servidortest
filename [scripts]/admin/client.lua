...
SVIDclient = Tunnel.getInterface("admin")
...
AddEventHandler("notifyExplosionStatus", function(status)
    explosionActive = status
    local message = explosionActive and "Alarme ativado!" or "Alarme desativado!"
    SetNotificationTextEntry("STRING")
    AddTextComponentString(message)
    DrawNotification(false, true)
end)

RegisterNetEvent("updateExplosionLocation")
AddEventHandler("updateExplosionLocation", function(location)
    explosionLocation = location
end)

RegisterNetEvent("triggerExplosion")
AddEventHandler("triggerExplosion", function(location)
    if not explosionActive then return end -- Evita explosões quando está desativado

    local playerPed = PlayerPedId()
    local playerCoords = GetEntityCoords(playerPed)
    local distance = #(playerCoords - vector3(location.x, location.y, location.z))

    if distance <= 35.0 and not IsEntityDead(playerPed) then
        -- Criar explosão visual sem fogo
