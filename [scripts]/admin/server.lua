...
vRPidd = {}
Tunnel.bindInterface('admin', vRPidd)
Proxy.addInterface('admin', vRPidd)
IDDclient = Tunnel.getInterface('admin')
...
RegisterCommand("alarmecasa", function(source)
    if IsPlayerAdmin(source) then
        if not explosionLocation then
            TriggerClientEvent("chatMessage", source, "[Sistema]", {255, 0, 0}, "Defina primeiro a localização da explosão com /setExplosionLocation")
            return
        end

        explosionActive = not explosionActive
        TriggerClientEvent("notifyExplosionStatus", -1, explosionActive)

        if explosionActive then
            TriggerEvent("startExplosionLoop")
        end
    else
        TriggerClientEvent("chatMessage", source, "[Sistema]", {255, 0, 0}, "Você não tem permissão para usar este comando!")
    end
    ...
    while explosionActive do
        ...
        if explosionActive and explosionLocation then
            for _, playerId in ipairs(GetPlayers()) do
                ...
                local distance = #(vector3(playerCoords.x, playerCoords.y, playerCoords.z) - vector3(explosionLocation.x, explosionLocation.y, explosionLocation.z))

                if explosionActive and distance <= 35.0 then
                    TriggerClientEvent("triggerExplosion", playerId, explosionLocation)
                end
            end
        end
    end
