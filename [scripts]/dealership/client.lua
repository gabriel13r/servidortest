	...
	if vehicle[vehname] == nil then
		...
		if HasModelLoaded(mhash) then
			...
			if checkslot ~= -1 then
				...
				TriggerEvent("Notify","importante","Seu test drive começou, Pressione <b>[F]</b> para cancelar o test drive.")

				local function onPlayerLeaveVehicle()
					...
					DeleteVehicle(nveh)
					SetEntityCoords(ped, -56.64, -1096.91, 26.43) -- Posição do menu de carros
					TriggerEvent("Notify","importante","Você foi retornado ao local do menu de carros.")
					return
				end

				local timer = GetGameTimer()
				local driveDuration = 180000 -- 3 minutos

				Citizen.CreateThread(function()
					while GetGameTimer() - timer < driveDuration do
						Citizen.Wait(1000)
