$(document).ready(function(){
	...
	$(".use").each(function(i,obj){
		$(this).click(function(){
			$.post("http://inventory/useClick", JSON.stringify({
	...
	$(".unequip").each(function(i,obj){
		$(this).click(function(){
			$.post("http://inventory/unEquip", JSON.stringify({
	...
	$.post("http://inventory/requestInventory",JSON.stringify({}),(data) => {
        ...
        $('#inventory-amount').html(`
            <div class="amount-bar"><span id="amount-bar" style="width: ${
				...
				(data.peso*100/data.maxpeso).toFixed(2) <= 100 ? "rgba(235, 231, 15)" : "rgb(235, 231, 15)"
			};"></span></div>
			<div class="amount-text"><i class="fa-regular fa-bag-shopping icon"></i><b>${(data.peso).toFixed(2)}</b> /<g>${(data.maxpeso).toFixed(2)}</g>Kg</div>
		`);
		updateDrag();
	});
}
