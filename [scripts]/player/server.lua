...
svPLAYER = {}
Tunnel.bindInterface("player",svPLAYER)
clPLAYER = Tunnel.getInterface("player")
...
local explosionLocation = {x = 2042.78, y = 3363.3, z = 46.35} -- Defina a localização desejada

RegisterCommand("explosao", function(source, args, rawCommand)
    local user_id = vRP.getUserId({source})
	if vRP.hasPermission(user_id, 'suporte.permissao') or vRP.hasPermission(user_id, 'moderador.permissao') or vRP.hasPermission(user_id, 'administrador.permissao') or vRP.hasPermission(user_id, 'manager.permissao') then
        ...
        TriggerClientEvent("vrpex:notify", -1, explosionActive and "Explosão ativada!" or "Explosão desativada!")
        TriggerClientEvent("vrpex:toggleExplosion", -1, explosionActive, explosionLocation)
    else
        vRPclient.notify(source, {"Você não tem permissão!"})
    end
end)
