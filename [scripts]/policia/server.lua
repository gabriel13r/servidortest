...
src = {}
Tunnel.bindInterface("policia",src)
...
RegisterServerEvent("policia:algemar")
AddEventHandler("policia:algemar",function()
	local source = source
	local user_id = vRP.getUserId(source)
	local nplayer = vRPclient.getNearestPlayer(source,2)
	if nplayer then
...
RegisterServerEvent("policia:carregar")
AddEventHandler("policia:carregar",function()
	local source = source
	local user_id = vRP.getUserId(source)
	local nplayer = vRPclient.getNearestPlayer(source,2)
