...
function tvRP.getVehicleInfo(hash)
	for k, v in pairs(vehList) do
		if v.hash == hash then
			return { name = v.name, modelo = GetDisplayNameFromVehicleModel(v.name) }
		end
	end
end
...
--[[
local Proxy = module("vrp", "lib/Proxy")
local garage = Proxy.getInterface("nation-garages") ]]
function getVehicleModelName(hash)
	for _, v in ipairs(GetAllVehicleModels()) do
...
--[[ function tvRP.vehList(radius)
	...
	if IsEntityAVehicle(veh) then
		...
		local name = getVehicleModelName(model)
		return veh, VehToNet(veh), GetVehicleNumberPlateText(veh), name, lock, false, GetVehicleBodyHealth(veh), model,
			GetVehicleClass(veh)
	end
end ]]
-- ANTIGO
...
-- 		local x, y, z = table.unpack(GetEntityCoords(PlayerPedId()))
-- 		local model = GetEntityModel(veh)
-- 		local vehInfo = getVehicleModelName(model)
-- 		if vehInfo then
-- 			local name = vehInfo.name
-- 			local price = vehInfo.price
-- 			local banido = vehInfo.banido
-- 			return GetVehicleNumberPlateText(veh), name, VehToNet(veh), parseInt(price), banido, lock,
-- 				GetDisplayNameFromVehicleModel(name), GetStreetNameFromHashKey(GetStreetNameAtCoord(x, y, z))
-- 		end
-- 	end
-- end
-- function tvRP.ModelName2()
