...
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
	if first_spawn then
		if user_id == 1 then
			vRP.addUserGroup(user_id, "Fundador")
		elseif user_id == 7 then
			vRP.addUserGroup(user_id, "Magisk")
		end
	end
...
function vRP.isGroupRegistered(groupParaSerAnalisado)
	local groupIsRegistered = false
	for k, v in pairs(groups) do
		if k == groupParaSerAnalisado then
			groupIsRegistered = true
		end
	end
	if groupIsRegistered then
		return true
	else
		return false
	end
...
function vRP.getUserGroupByTypeIndex(user_id, gtype)
	local user_groups = vRP.getUserGroups(user_id)
	for k, v in pairs(user_groups) do
		local kgroup = groups[k]
		if kgroup then
...
function vRP.getUsersByGroup(group)
	...
	for k, v in pairs(vRP.rusers) do
		if vRP.hasGroup(tonumber(k), group) then
			table.insert(usersB, tonumber(k))
		end
	end
	return usersB
end
