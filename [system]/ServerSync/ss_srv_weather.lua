...
function PushNextWeather()
    ...
    if string.upper(currentWeather) == string.upper("THUNDER") or
        string.upper(currentWeather) == string.upper("CLEARING") then
        rainTimer = ss_rain_timeout * 60
        rainPossible = false
    end

    if ss_show_console_output then
        TraceMsg("New Weather: " ..
            currentWeather ..
            " (Tried to reduce: " ..
            tostring(reduced) ..
            " from " .. reducedW ..
            ") PossibleRain: " .. tostring(rainPossible) .. " | rainTimer: " .. tostring(rainTimer))
    end
        ...
        if IsPlayerAceAllowed(source, "changeWeather") then
            ...
            if args[1] == nil then
                TriggerClientEvent('chatMessage', source, '', { 255, 255, 255 },
                    '^1Error: Invalid syntax, use ^0/clima <weatherType>!')
